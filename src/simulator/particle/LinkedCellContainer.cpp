...
std::vector<ParticlePointer> &LinkedCellContainer::get_neighbours(Particle &p) {
  ...
  if (domain_size_.size() == 2) {
    if (i == x - 1 && j == y - 1) {
      ...
      return get_particles_from_indices(
          {index - 1, index + x, index - x, index + x - 1, index - x - 1});
    } else if (j == y - 1) {
      return get_particles_from_indices(
          {index + 1, index - x, index - 1, index - x + 1, index - x - 1});
    } else if (i == 0) {
      return get_particles_from_indices(
          {index + 1, index + x, index - x, index + x + 1, index - x + 1});
    } else if (j == 0) {
      ...
      return get_particles_from_indices(
          {index + 1, index - 1, index + x, index - x, index + x + 1,
  ...
  // handle 3d case (this is going to be messy)
    ...
    if (i == x - 1 && j == y - 1 && k == z - 1) { // top, right, front
      return get_particles_from_indices(
          {index - 1, index - x, index - x - 1, index - x * y,
           index - x * y - 1, index - x * y - x, index - x * y - x - 1});
      ...
      return get_particles_from_indices(
          {index + 1, index + x, index + x + 1, index - x * y,
           index - x * y + 1, index - x * y + x, index - x * y + x + 1});
    } else if (i == 0 && j == 0 && k == 0) { // bottom,left,back
      return get_particles_from_indices(
          {index + 1, index + x, index + x + 1, index + x * y,
           index + x * y + 1, index + x * y + x, index + x * y + x + 1});
    }
