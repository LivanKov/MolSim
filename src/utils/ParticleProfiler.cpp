...
void ParticleProfiler::apply_profiler() {
    ...
    if (!file.is_open()) {
        Logger::getInstance().error("Error: Unable to open file for writing" + output_file_);
        return;
    }
    ...
    for (size_t i = 0; i < particles_.size(); ++i) {
        ...
        if (x_position >= x_min_ && x_position <= x_max_) {
            ...
            // when you have an x at x_max, bin_index would be 1 too large -> so reduce by 1
    ...
    for (size_t i = 0; i < number_of_bins_; ++i) {
        ...
        if (n_particles_bin > 0) {
            ...
            double avg_velo_x = total_velocity[i][0] / static_cast<double>(n_particles_bin);
            double avg_velo_y = total_velocity[i][1] / static_cast<double>(n_particles_bin);
            double avg_velo_z = total_velocity[i][2] / static_cast<double>(n_particles_bin);

            // Get the x-position of the center of the bin
            double bin_center_x = x_min_ + (i + 0.5) * bin_width_;

            // Write the data to the CSV file
            file << i << "," << bin_center_x << "," << density << ","
                 << avg_velo_x << "," << avg_velo_y << "," << avg_velo_z << "\n";
        }
    }
